...
using xBuild.Shell;
using xBuild.Shell.DotnetTools;
using xBuild.Targets;

namespace _build.Targets;

public class StaticAnalysisTargets(
    DotnetToolsGeneration dotnetToolsGeneration
) : ITargetProvider
{
    ITarget Cleanup => field ??= new Target(description: "Cleans up code")
        ...
        .JetbrainsResharperGlobaltools($"cleanupcode {BuildConstants.SolutionPath}");
    ...
    public ITarget Inspect => field ??= new Target(description: "Inspects code for issues")
        ...
        .Executes(async (ITargetLogger logger, TargetShell shell, CancellationToken stoppingToken) =>
        {
            var log = await shell
                ...
                .JetbrainsResharperGlobaltools($"inspectcode {BuildConstants.SolutionPath} -stdout")
...
//                          """
//                     )
                var results = await shell.NugetLicense(
                        $"""
                         -i {BuildConstants.SolutionPath} 
                         --include-transitive 
                         --output JsonPretty 
                         --allowed-license-types allowed-licenses.json
                         -override override-licenses.json
